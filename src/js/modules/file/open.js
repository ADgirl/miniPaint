...
class File_open_class {
    ...
    open_file() {
        ...
        alertify.success('您也可以将项目拖放到浏览器中。');
    ...
    open_webcam() {
        ...
        function handleError(error) {
            alertify.error('抱歉, 无法加载 getUserMedia() data: ' + error);
        }
    ...
    file_open_data_url_handler(data) {
        ...
        img.onerror = function(ex) {
            alertify.error('对不起，图片无法加载。尝试复制图像并粘贴。');
        };
        img.src = data;
    }
    ...
    open_handler(e) {
        ...
        for (var i = 0, f; i < files.length; i++) {
            ...
            if (!f.type.match('image.*') && !f.name.match('.json')) {
                alertify.error('错误的文件类型，必须是图像或JSON。');
                continue;
            }
            if (files.length == 1)
